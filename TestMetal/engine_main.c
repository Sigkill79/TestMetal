...
int engine_load_assets(EngineStateStruct* engine) {
    ...
    entity_set_position(entity, vec3(0.0f, 0.0f, -2.0f)); // Position in front of camera
...
void engine_update(EngineStateStruct* engineState) {
    ...
    fprintf(stderr, "Engine state: %p, state: %d\n", engineState, engineState->state);
    ...
    if (engineState->state == ENGINE_STATE_RUNNING) {
        ...
        if (engineState->world && engineState->world->entity_count > 0) {
            ...
            if (entity) {
                ...
                fprintf(stderr, "Updated entity '%s' rotation (time: %.3f)\n", 
                        entity_get_name(entity), rotation_time);
            }
        }
